...
The extension passes authentication and refresh to the installed AWS CLI. It never logs raw CLI output, persists credential values, or sends AWS data to the author. Its local cache stores only status/profile metadata and retry/reminder state. See the README for the complete security model and limitations.
